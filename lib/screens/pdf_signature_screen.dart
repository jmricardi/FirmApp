...
class _PdfSignatureScreenState extends State<PdfSignatureScreen> {
  ...
  double _currentSigAspectRatio = 1.8; // Default 180/100
  ...
  Future<void> _selectSignature(File sig) async {
    final bytes = await sig.readAsBytes();
    ...
    if (mounted) {
      setState(() {
        ...
        _currentSigAspectRatio = aspectRatio;
        // Ajustamos la altura para mantener el aspect ratio basado en el ancho actual
        _currentSigHeightInPoints = _currentSigWidthInPoints / aspectRatio;
      });
    }
    image.dispose();
  }
  ...
  Future<void> _initData() async {
    try {
      ...
      if (_availableSignatures.isNotEmpty) {
        await _selectSignature(_availableSignatures.first);
      }
  ...
  Future<void> _loadPage(int index) async {
    ...
    try {
      ...
      if (pageImage != null) {
        ...
        debugPrint(
            "UI_RENDER: requested=${w}x$h | actual=${uiImage.width}x${uiImage.height} | pdfPts=${page.width}x${page.height}");
        debugPrint(
            "UI_RENDER AR: page=${page.height / page.width} | image=${uiImage.height / uiImage.width}");
  ...
  void _addStamp() {
    ...
    debugPrint(
        "DEBUG STAMP ADDED: Pos=${_currentSigPosInPoints.dx},${_currentSigPosInPoints.dy} | PageSize=${_pdfPageSize?.width}x${_pdfPageSize?.height}");
  }
  ...
  Future<void> _saveFinalPdf() async {
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text(
                "Esto puede tardar unos segundos dependiendo del número de páginas.",
    ...
    try {
      final success = await creditService.useCredit(
          amount: 1, description: "Firma de Documento PDF");
      ...
      for (int i = 0; i < _totalPages; i++) {
        ...
        final pageBytes =
            await pageUiImg.toByteData(format: ui.ImageByteFormat.png);
        final bgImage = pw.MemoryImage(pageBytes!.buffer.asUint8List());

        debugPrint(
            "EXPORT PAGE[$i]: pdfPts=${originalPageW}x$originalPageH | renderPx=${renderW}x$renderH | actualPx=${pageUiImg.width}x${pageUiImg.height}");
        ...
        if (_stamps.containsKey(i)) {
          for (var stamp in _stamps[i]!) {
            ...
            sigCanvas.drawImageRect(
                sigUiImg,
                Rect.fromLTWH(0, 0, sigUiImg.width.toDouble(),
                    sigUiImg.height.toDouble()),
                Rect.fromLTWH(0, 0, sigUiImg.width.toDouble(),
                    sigUiImg.height.toDouble()),
                Paint()
                  ..colorFilter =
                      ui.ColorFilter.mode(stamp.color, ui.BlendMode.srcIn)
                  ..filterQuality = ui.FilterQuality.high);
            final coloredSigImg = await sigRecorder
                .endRecording()
                .toImage(sigUiImg.width, sigUiImg.height);
            final coloredSigBytes =
                await coloredSigImg.toByteData(format: ui.ImageByteFormat.png);
            final sigPwImage =
                pw.MemoryImage(coloredSigBytes!.buffer.asUint8List());

            debugPrint(
                "STAMP[$i]: pos=(${stamp.positionInPoints.dx}, ${stamp.positionInPoints.dy}) | size=(${stamp.widthInPoints}x${stamp.heightInPoints}) | page=(${originalPageW}x$originalPageH)");
      ...
      final originalName = widget.pdfFile.path
          .split(Platform.pathSeparator)
          .last
          .split('.')
          .first;
      final fileName =
          'FF_SIGNED_${originalName}_${DateTime.now().millisecondsSinceEpoch}.pdf';
      ...
      if (mounted) {
        ...
        Navigator.pop(
            context, true); // Volver al Home indicando que hubo cambios
      }
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: Builder(builder: (context) {
        ...
        Expanded(child: LayoutBuilder(builder: (context, constraints) {
          ...
          final sheetHeight =
              sheetWidth * (_pdfPageSize!.height / _pdfPageSize!.width);
                ...
                child: Center(
                  child: Container(
                    ...
                    child: Stack(clipBehavior: Clip.none, children: [
                      if (_pageImage != null)
                        SizedBox.expand(
                            child: Image.memory(_pageImage!, fit: BoxFit.fill)),

                      if (_stamps.containsKey(_currentPageIndex))
                        ..._stamps[_currentPageIndex]!
                            .map((s) => _buildStamp(s, scaleX, scaleY)),
                      ...
                      if (_selectedSignature != null)
                        Positioned(
                          ...
                          child: GestureDetector(
                            onPanStart: (details) {
                              final RenderBox? box = _pageContainerKey
                                  .currentContext
                              ...
                              debugPrint(
                                  "DRAG_START (Sig): localTouch=$localTouch | sigCorner=$sigCorner");
                            },
                            onPanUpdate: (details) {
                              ...
                              final Offset localTouch =
                                  box.globalToLocal(details.globalPosition);
                              final Offset sigCorner =
                                  localTouch - _dragStartGlobal!;

                              final double pdfX = (sigCorner.dx / scaleX).clamp(
                                  0,
                                  _pdfPageSize!.width -
                                      _currentSigWidthInPoints);
                              final double pdfY = (sigCorner.dy / scaleY).clamp(
                                  0,
                                  _pdfPageSize!.height -
                            ...
                            child: Container(
                              ...
                              child: ColorFiltered(
                                  colorFilter: ColorFilter.mode(
                                      _inkColor, BlendMode.srcIn),
                                  child: Image.file(_selectedSignature!,
                                      fit: BoxFit.fill)),
                            ),
                          ),
                        ),
  ...
  Widget _buildStamp(SignatureStamp stamp, double scaleX, double scaleY) {
    return Positioned(
        ...
        child: SizedBox(
            width: stamp.widthInPoints * scaleX,
            height: stamp.heightInPoints * scaleY,
            child: ColorFiltered(
                colorFilter: ColorFilter.mode(stamp.color, BlendMode.srcIn),
                child: Image.file(stamp.signatureFile, fit: BoxFit.fill))));
  }
    ...
    return GestureDetector(
        ...
        child: Container(
            ...
            decoration: BoxDecoration(
                ...
                border: Border.all(
                    color: _inkColor == color ? Colors.white : Colors.white24,
                    width: 2),
