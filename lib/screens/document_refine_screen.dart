...
class _DocumentRefineScreenState extends State<DocumentRefineScreen> {
  ...
  Future<void> _suggestFormat() async {
    try {
      ...
      final double a4AR =
          PdfPageFormat.a4.width / PdfPageFormat.a4.height; // ~0.707
      final double letterAR =
          PdfPageFormat.letter.width / PdfPageFormat.letter.height; // ~0.772
      final double legalAR =
          PdfPageFormat.legal.width / PdfPageFormat.legal.height; // ~0.607
      ...
      PdfPageFormat suggested =
          diffs.entries.reduce((a, b) => a.value < b.value ? a : b).key;
  ...
  Future<void> _finish() async {
    ...
    try {
      ...
      if (_screenW > 0 && _screenH > 0 && _frameW > 0 && _frameH > 0) {
        ...
        final bottomRightChild =
            MatrixUtils.transformPoint(mInv, cropRect.bottomRight);
        ...
        int cropW =
            ((bottomRightChild.dx - topLeftChild.dx) / fitScale).round();
        int cropH =
            ((bottomRightChild.dy - topLeftChild.dy) / fitScale).round();
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: isSelected ? Colors.white : Colors.transparent, width: 1),
        ),
        child: Text(label,
            style: TextStyle(
...
class _CropOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
        Rect.fromLTWH(rect.left - 2, rect.top - 2, cs, 4), cornerPaint);
    canvas.drawRect(
        Rect.fromLTWH(rect.left - 2, rect.top - 2, 4, cs), cornerPaint);

    canvas.drawRect(
        Rect.fromLTWH(rect.right - cs + 2, rect.top - 2, cs, 4), cornerPaint);
    canvas.drawRect(
        Rect.fromLTWH(rect.right - 2, rect.top - 2, 4, cs), cornerPaint);

    canvas.drawRect(
        Rect.fromLTWH(rect.left - 2, rect.bottom - 2, cs, 4), cornerPaint);
    canvas.drawRect(
        Rect.fromLTWH(rect.left - 2, rect.bottom - cs + 2, 4, cs), cornerPaint);

    canvas.drawRect(Rect.fromLTWH(rect.right - cs + 2, rect.bottom - 2, cs, 4),
        cornerPaint);
    canvas.drawRect(Rect.fromLTWH(rect.right - 2, rect.bottom - cs + 2, 4, cs),
        cornerPaint);
  }
